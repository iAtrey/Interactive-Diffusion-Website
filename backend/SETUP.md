...
| `DIFFATS_BACKEND` | `~/backend` | Where you've placed `main.py`, `run_generation.py`, etc. Needs to be manually added using the Nano command.|

Example for a machine where the repo, checkpoint, environment, and backend
code live somewhere other than the defaults:
...
You need one Python environment with torch and CUDA working, and it must be
reachable by an absolute path (never activated implicitly. all stages are
launched as `<that path> script.py ...`, not `python script.py`).

If your system alr has a working `torch` with
...
rather than "whatever's newest." Go to the comments at the top of that file
for the extra `--index-url` it needs, because CUDA-tagged torch builds aren't
on plain PyPI.
...
Python version. That environment is unrelated to serving videos. 
...
checkpoint (mines is called `instructor_test`, since that's this
project's folder name for it. rename freely if yours is called something
else. Adapting To A Different Layout has more information to help below.).


**Important**


That set has two real parameter setups:

- **Setup A** : niu=0.021, Re=76.19, cx=27, cy=58, r=8
- **Setup B** : niu=0.023, Re=69.57, cx=30, cy=64, r=8

- Setup A has ID's 0-49 and B 50-99.

These aren't freely editable, though. You can generate new setups by yourself through `generate_data.py.`



## Adapting to a different data layout

the four env vars are general. What isn't environment-driven is the internal folder structure under
...
- serves the frontend itself at `/`, so the browser and the API share one
  origin — no cross-origin requests, no CORS complexity
...
tracker (`job_manager.py`) holds exactly one job's state globally.
